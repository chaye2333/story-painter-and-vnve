...
import { Loader2, Plus, Split, FileText } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="text-xs text-muted-foreground bg-muted/50 p-2 rounded">
        提示：点击行号旁的 <Plus className="inline w-3 h-3" /> 按钮可以在该行前插入新的场景分割点。
      </div>
          ...
          {lines.map((line, i) => {
             ...
             return (
               <div key={i} className={cn(
                 ...
                 (isHeader || isSceneName) && "bg-primary/10 font-bold",
                 isSplitPoint && "border-l-primary mt-4 border-t border-t-primary/20"
               )}>
                  ...
                  <div className="relative w-6 h-6 mr-2 flex-shrink-0 flex items-center justify-center">
                     <Button 
                       size="icon" 
                       variant="ghost" 
                       className="h-6 w-6 opacity-20 group-hover:opacity-100 transition-all hover:bg-primary hover:text-primary-foreground" 
                       title="在此行前插入新场景"
